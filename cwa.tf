resource "aws_cloudwatch_metric_alarm" "svc_alarm_aurora_cpu" {
  ...
  alarm_actions = [
    local.is_prod ? "arn:aws:sns:${var.aws_region}:${data.aws_caller_identity.current}:cloudwatch-alarm-funnel" : "arn:aws:sns:${var.aws_region}:${data.aws_caller_identity.current}:cloudwatch-alarm-funnel",
  ]
